...
class PlayerResetPasswordBaleNotificationTests(TestCase):
    ...
    Requests are pinned to English throughout, reverse() included: the
    resulting messages now have real Persian translations, and
    i18n_patterns' URL-prefix ("/fa/..." vs "/en/...") determines the
    active language for the request regardless of any override that
    doesn't also cover which URL got resolved — so reverse() has to run
    inside the same override as the request that follows it.
    """
